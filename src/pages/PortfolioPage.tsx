...
const PortfolioPage = () => {
  ...
  if (error) {
    return <div className="text-center p-6 text-red-500">
  ...
  return <div className="container mx-auto p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="relative w-full max-w-md">
          <Input type="text" placeholder="Search companies by name, industry..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-4 pr-10 py-2 w-full" />
      ...
      {filteredCompanies.length === 0 ? <div className="text-center p-10 bg-gray-50 rounded-lg">
          <p className="text-gray-500">No companies found matching your search criteria.</p>
        </div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCompanies.map(company => <Card key={company.id} className="overflow-hidden border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-start gap-4 mb-4">
                  <div className="flex justify-center items-center bg-purple-100 rounded-lg" style={{
              width: 120,
              height: 146,
              minWidth: 120,
              minHeight: 146
            }}>
                    {company.logo_url ? <img src={company.logo_url} alt={`${company.name} logo`} style={{
                width: 120,
                height: 146,
                borderRadius: '0.5rem',
                background: '#F4EBFF'
              }} className="object-scale-down" /> : <Building className="h-8 w-8 text-purple-600" />}
                  </div>
                  <div>
                ...
                <div className="space-y-2 mb-4">
                  {company.founded_year && <div className="flex items-center text-sm text-gray-600">
                      <User className="h-4 w-4 mr-2" />
                      <span>Founded: {company.founded_year}</span>
                    </div>}
                  {company.website && <div className="flex items-center text-sm text-gray-600">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                  <Button variant="outline" className="text-purple-600 border-purple-200 hover:bg-purple-50" onClick={() => handleOpenIntroDialog(company)}>
                    Request Intro
                  </Button>
                  <Button variant="ghost" className="flex items-center gap-1" onClick={() => handleViewDetails(company)}>
          ...
          {selectedCompany && <>
              ...
              <div className="mt-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                  {selectedCompany.founded_year && <div className="border p-3 rounded-md">
                  ...
                  {selectedCompany.investment_year && <div className="border p-3 rounded-md">
                  ...
                  {selectedCompany.website && <div className="border p-3 rounded-md">
                      <h4 className="text-xs text-gray-500 mb-1">Website</h4>
                      <div className="flex items-center">
                        <Globe className="h-4 w-4 mr-2 text-purple-600" />
                        <a href={selectedCompany.website.startsWith('http') ? selectedCompany.website : `https://${selectedCompany.website}`} target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:underline">
